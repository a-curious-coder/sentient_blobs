...
from modules.drawer import draw_stats
from modules.game_event_handler import handle_keydown, handle_mousebuttondown, quit_game
from modules.getters import *
from modules.utilities.boundary_shape import Rectangle
from modules.utilities.collision_logic import check_collision, player_eaten_player
...
def main(config_file):
    ...
    config = modules.neat.config.Config(modules.neat.DefaultGenome, 
                                modules.neat.DefaultReproduction,
                                modules.neat.DefaultSpeciesSet, 
                                modules.neat.DefaultStagnation,
                                config_file)
    
    config.genome_config.initial_connection = "full_nodirect"

    # Create a modules.neat.population.Population object using the Config object created above
    neat_pop = modules.neat.population.Population(config)
    
    # Show the summary statistics of the learning progress
    neat_pop.add_reporter(modules.neat.StdOutReporter(True))
